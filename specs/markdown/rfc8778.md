...
This document speciﬁes the conventions for using the Hierarchical Signature System (HSS) /
Leighton-Micali Signature (LMS) hash-based signature algorithm with the CBOR Object Signing
and Encryption (COSE) syntax. The HSS/LMS algorithm is one form of hash-based digital
...
3.  Hash-Based Signature Algorithm Identiﬁers
...
Leighton-Micali Signature (LMS) hash-based signature algorithm with the CBOR Object Signing
and Encryption (COSE)
 syntax. The LMS system provides a one-time digital signature
that is a variant of Merkle Tree Signatures (MTS). The HSS is built on top of the LMS system to
eﬃciently scale for a larger number of signatures. The HSS/LMS algorithm is one form of a hash-
based digital signature, and it is described in
only be used for a ﬁxed number of signing operations. The number of signing operations
...
The HSS/LMS signatures

 are currently deﬁned to use exclusively SHA-256

. The HSS/LMS signature algorithm can
...
discrete logarithm and factoring cannot be depended upon.
 ...
 pose a threat to widely deployed digital signature algorithms. As a result,

 and progress in the development of quantum
...
open to conjecture; however, RSA
, DSA
Algorithm (ECDSA)
...
Since the HSS/LMS signature algorithm does not depend on the diﬃculty of discrete logarithm or
...
This speciﬁcation makes use of the hash-based signature algorithm speciﬁed in
which is the Leighton and Micali adaptation
Merkle one-time signature system

[HASHSIG]
,
 of the original Lamport-Diﬃe-Winternitz-
.
...
hash function. The hash-based signature algorithm speciﬁed in
of the SHA-256 one-way hash function
the registration of additional one-way hash functions in the future.

[HASHSIG]
, but it also establishes an IANA registry to permit
...
The hash-based signature algorithm speciﬁed in
...
the signer. Otherwise, generation of the entire tree might take weeks or longer.
 ...
 uses a hierarchy of trees. The N-time

[HASHSIG]

[HASHSIG]

An HSS signature, as speciﬁed in
...
, a signed_public_key is the lms_signature over the public
...
(LMS) system. LMS systems have two parameters. The ﬁrst parameter is the height of the tree, h,
which is the number of levels in the tree minus one. The
...
tree. The second parameter is the number of bytes output by the hash function, m, which is the
 speciﬁcation supports only
amount of data associated with each node in the tree. The
SHA-256 with m=32. An IANA registry is deﬁned so that other hash functions could be used in the
future.

 includes support for ﬁve
...
[HASHSIG]
 ...
 speciﬁcation supports ﬁve tree sizes:
...
hash functions and additional tree sizes in the future.
 ...
 speciﬁcation establishes an IANA registry to permit the registration of additional

[HASHSIG]

 speciﬁcation deﬁnes the value I as the private key identiﬁer, and the same I value

The
is used for all computations with the same LMS tree. The value I is also available in the public
key. In addition, the
associated with the ith node in the LMS tree, and the nodes are indexed from 1 to 2^(h+1)-1.
Thus, T[1] is the m-byte string associated with the root of the LMS tree.

 speciﬁcation deﬁnes the value T[r] as the m-byte string
...
, the LMS signature consists of four elements:
...
an array of values that is associated with the path through the tree from the leaf associated
...
values. The ﬁrst value is the sibling of the leaf, the next value is the sibling of the parent of the
...
The hash-based signature algorithm depends on a one-time signature method. This speciﬁcation
makes use of the Leighton-Micali One-time Signature (LM-OTS) Algorithm
has ﬁve parameters:
...
A preimage-resistant hash function that accepts byte strings of any length and returns an
n-byte string.

The width in bits of the Winternitz coeﬃcients. [HASHSIG] supports four values for this
parameter: w=1, w=2, w=4, and w=8.

The number of n-byte string elements that make up the LM-OTS signature.

The number of left-shift bits used in the checksum function, which is deﬁned in
of [HASHSIG]
...
[HASHSIG]
 ...
 speciﬁcation supports four LM-OTS variants:
...
The LM-OTS signature value can be summarized as the identiﬁer of the LM-OTS variant, the
random value, and a sequence of hash values (y[0] through y[p-1]), as described in
:
[HASHSIG]
...
The CBOR Object Signing and Encryption (COSE)
 supports two signature algorithm
schemes. This speciﬁcation makes use of the signature with appendix scheme for hash-based
signatures.
...
MUST
 ...
 include 'sign' when creating a hash-based signature.
...
MUST
 ...
 include 'verify' when verifying a hash-based

•

 be used to identify the top of the HSS tree. In [HASHSIG],
If the 'kid' ﬁeld is present, it
this identiﬁer is called 'I', and it is the 16-byte identiﬁer of the LMS public key for the tree.
...
Implementations
the ability to forge signatures. Along with the private key, the implementation
...
space. The generation of quality random numbers is diﬃcult, and
...
System (HSS). In the absence of a public key infrastructure
...
maintains state across diﬀerent invocations of the signing algorithm.
Section 9.2 of [HASHSIG]
oﬀers some practical implementation approaches around this statefulness. In some of these
approaches, nodes are sacriﬁced to ensure that none are used more than once. As a result, the
...
tree nodes have been used is deliberately not deﬁned. It was not deﬁned to avoid creating the
...
McGrew, D., Curcio, M., and S. Fluhrer "Leighton-Micali Hash-Based Signatures"
...
FIPS Publication 180-4 DOI 10.6028/NIST.FIPS.180-4
...
Ptacek, T., Ritter, T., Samuel, J., and A. Stamos "The Factoring Dead: Preparing
...
,
Standard (DSS)" FIPS Publication 186-4 DOI 10.6028/NIST.FIPS.186-4
...
Leighton, F. and S. Micali "Large provably fast and secure digital signature
,
...
, 1988,
Science, Volume 291 DOI 10.1007/3-540-48184-2_32
...
Merkle, R. "A Certiﬁed Digital Signature" Advances in Cryptology -- CRYPTO '89
,
Proceedings Lecture Notes in Computer Science, Volume 435 DOI
...
,
,
CRYPTO '89 Proceedings Lecture Notes in Computer Science, Volume 435 DOI
...
,
Bernstein, D. "Introduction to post-quantum cryptography" DOI
, 2009,
...
example of a COSE_Sign1 message. This section is formatted according to the extended CBOR
diagnostic format deﬁned by
...
The size of binary ﬁle is 2560 bytes.
...
The size of binary ﬁle is 2552 bytes.
...
Many thanks to
,
John Mattsson Jim Schaad
an extra special thank you to

Roman Danyliw Elwyn Davies Scott Fluhrer Ben Kaduk Laurence Lundblade

,
 for their valuable review and insights. In addition,
...
Jim Schaad
 ...
 for generating the examples in Appendix A.
