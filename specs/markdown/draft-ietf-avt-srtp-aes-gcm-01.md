...
Internet Draft         AES-GCM and AES-CCM for SRTP         Jan 26, 2011
...
1.2.2. Values used to form the Initialization Vector (IV)
   ...
   The initialization vector for an SRTP packet is formed from the:
     ...
     Packet Counter     Each AEAD instantiation MUST maintain a 6 octet
                        ...
                        closely related to the invocation field
                        discussed in NIST Special Publication 800 38-D
                        [GCM], "Recommendation for Block Cipher Modes of
                        Operation: Galois/Counter Mode (GCM) and GMAC".
                        As we shall see below, the packet counter is
                        used to insure each packet gets a unique
...
Internet Draft         AES-GCM and AES-CCM for SRTP         Jan 26, 2011
     ...
     SALT               A 12-octet SRTP session encryption salt produced
                        by the SRTP Key Derivation Function (KDF).

1.2.3. SRTP IV formation for AES-GCM and AES-CCM

   AES-GCM and AES-CCM SRTP use a 12 byte inititialition vector which is
   formed as follows.  A 12-octet string is formed by concatenating a
   2-octets of zeroes, the 4-octet SSRC, and the the 6-byte invocation
...
1.2.4. SRTCP IV formation for AES-GCM and AES-CCM
   ...
   Derivation Function (KDF) as described in [RFC3711].  (The 31-bit
...
1.2.6.1. Encrypted SRTCP packets
   ...
   encryption flag and SRTCP index are treated as AAD and eight octets
...
4. IANA Considerations
   ...
   which corresponds to the use of an AEAD algorithm in SRTP.  In
   order to allow the use of the algorithms defined in this document
