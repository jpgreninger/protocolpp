...
PRELIMINARIES .............................................................................................................................................6
UNDERLYING BLOCK CIPHER ALGORITHM .................................................................................................6
CRYPTOGRAPHIC PRIMITIVES......................................................................................................................7
DATA ELEMENTS.........................................................................................................................................7
INPUT FORMATTING ....................................................................................................................................8
EXAMPLES OF OPERATIONS AND FUNCTIONS ..............................................................................................8
CCM SPECIFICATION ...................................................................................................................................9
GENERATION-ENCRYPTION PROCESS ..........................................................................................................9
DECRYPTION-VERIFICATION PROCESS ......................................................................................................10
APPENDIX A:   EXAMPLE OF A FORMATTING AND COUNTER GENERATION FUNCTION.............12
LENGTH REQUIREMENTS ...........................................................................................................................12
FORMATTING OF THE INPUT DATA ............................................................................................................12
...
FORMATTING OF THE COUNTER BLOCKS...................................................................................................14
APPENDIX B:  LENGTH OF THE MAC ..............................................................................................................15
AUTHENTICATION ASSURANCE .................................................................................................................15
SELECTION OF THE MAC LENGTH.............................................................................................................16
...
Input  data  to  the  CCM  generation-encryption  process  that  is
...
A  bit  string  whose  length  is  the  block  size  of  the  block  cipher
algorithm.

A  family  of  functions  and  their  inverses  that  is  parameterized  by
cryptographic keys; the functions map bit strings of a fixed length to
...
The process of CCM in which a purported ciphertext is decrypted and
...
The function that transforms the payload, associated data, and nonce
...
The  process  of  CCM  in  which  a  MAC  is  generated  on  the  payload
...
The  inverse  function  of  the  forward  cipher  function  for  a  given
cryptographic key.
...
features a symmetric key block cipher algorithm.
...
The input data to the CCM generation-encryption process that is both
authenticated and encrypted.

Valid Data Element

A  payload,  an  associated  data  string,  or  a  nonce  that  satisfies  the
...
The  maximum  number  of  times  that  the  output  of  any  implementation  of  the
...
The  highest  acceptable  probability  for  an  inauthentic  message  to  pass  the
decryption-verification process.

The octet length of the MAC.

The MAC that is generated as an internal variable in the CCM
processes.
...
The output of the forward cipher function of the block cipher algorithm under the
...
data blocks, denoted BB0, B1B , …, BBr for some non-negative integer r, in accordance with a function
...
1.  The first block, BB0, uniquely determines the nonce N.
2.  The formatted data uniquely determines P and A; moreover, if (N, P, A) and (N, P', A')
', then BBi is

are distinct input triples whose formatting is BB0, B1B , …, BBr and B0B ', BB1', …, Br'
distinct from BiB ' for some index i such that i ≤ r and i ≤ r '. ∗

3.  The first block, BB

0, is distinct from any counter blocks that are used across all invocations
...
Apply the formatting function to (N, A, P) to produce the blocks BB0, B1B , …, BBr.
Set Y0= CIPHK(BB0).
For i = 1 to r, do Yi  =  CIPHK(BBi ⊕ Yi-1).
...
Set T=MSBTlen(Yr).
Apply  the  counter  generation  function  to  generate  the  counter  blocks Ctr0,  Ctr1,
…, Ctrm, where
m =
For j=0 to m, do S  j = CIPHK(Ctrj).
Set S= S1 || S2 || …|| Sm.
Return C=(P ⊕ MSBPlen(S)) || (T ⊕ MSBTlen(S0)).
...
Set T=LSBTlen(C) ⊕ MSBBTlen(S0).
If N, A, or P is not valid, as discussed in Section 5.4, then return INVALID, else
apply the formatting function to (N, A, P) to produce the blocks BB0, B1B , …, BBr.
Set Y0= CIPHK(BB0).
For i = 1 to r, do Yj  =  CIPHK(BBi ⊕ Yi-1).
...
The formatting of the input data (N, A, P) into a sequence of blocks BB0, B1B , …, BBr is presented in
...
The  leading  octet  of  the  first  block  of  the  formatting,  BB0,  contains  four  flags  for  control
...
Table 1: Formatting of the Flags Octet in BB0
...
If a=0, as indicated by the Adata field in the first octet of BB0, then there are no blocks devoted to
...
These blocks are denoted in the formatted data as B1B , BB2, … BuB  for some positive integer u that
depends on a.
...
formatted data as BBu+1, BBu+2 … BBr, where r=u+ ⎡
...
also be set to 0, to ensure that all the counter blocks are distinct from BB0 (as specified in A.2.1
above). Bits 0, 1, and 2 contain the same encoding of q as in B0B .
...
D.  Whiting,  R.  Housley,  N.  Ferguson,  Counter  with  CBC-MAC  (CCM).  Available  at
http://csrc.nist.gov/ CryptoToolkit/modes/proposedmodes/.

FIPS Publication 197, Advanced Encryption Standard (AES). U.S. DoC/NIST, November
26, 2001.  Available at http://csrc.nist.gov/publications/.

FIPS  Publication  46-3,  Data  Encryption  Standard  (DES).  U.S.  DoC/NIST,  October  25,
