...
Intended Applications for Stateful HBS Schemes ........................................... 1
...
eXtended Merkle Signature Scheme (XMSS) Parameter Sets .......................... 16
...
Intended Applications for Stateful HBS Schemes
...
Glossary of Terms, Acronyms, and Mathematical Symbols
...
T192(SHA-256(M)), the most significant (i.e., leftmost) 192 bits of the
SHA-256 hash of M.
...
A truncation function that outputs the most significant (i.e., leftmost)
...
In LMS, the number of bytes associated with each node of a Merkle
tree.

1.  In XMSS, the length of a Winternitz chain. A single Winternitz chain
...
The number of n-byte string elements in an LM-OTS private key, public
key, and signature.

The number of n-byte string elements in a WOTS+ private key, public
key, and signature.

In LMS and XMSS, the height of the tree. In XMSSMT, the total height
...
A secret random value used for pseudorandom key generation in
XMSS.

A 32-byte data structure used in XMSS when generating prefixes (keys)
and bitmasks.

1. In XMSS, the public, random, unique identifier for the long-term key.
...
signature scheme, the message to be signed is hashed to create a digest; the digest is encoded as a
 number;1 and then each digit of the digest is signed using a hash chain, as follows.
base
...
b
 ...
 should generally correspond to the targeted security strength of the scheme. So, for

b

A hash chain is created by first randomly generating a secret value,
The size of
the parameter sets approved by this recommendation,
The public key,

x
, is then created by applying the hash function,

x
. Figure 1 shows an example of a hash chain for the

x

, which is the private key.

 will be either 192 or 256 bits in length.
 times,
, to the secret
 is 4.
th digit of a digest where
b  – 1
b
...
𝑝𝑝𝑝𝑝𝑝𝑝𝑘𝑘 = 𝐻𝐻 �𝐻𝐻�𝐻𝐻(𝑥𝑥𝑘𝑘)��

, is signed by applying the hash function,

. Figure 2 shows an example of a signature for the

th digit of the digest,

The
times,
k
using the Winternitz chain in Figure 1 when

N
𝐻𝐻
. Figure 2 also shows how the signature,
k
...
applied to
𝐻𝐻(𝑥𝑥𝑘𝑘)
signature is valid. In general, the signature for the
checking that
...
, to the private key
th digit of the digest created

k

N

 is 1. As shown, the signature is
...
, is
, can be verified. The hash function,
1
𝑠𝑠𝑘𝑘 = 𝐻𝐻
(𝑥𝑥𝑘𝑘) =
, then the
H
th digit of a digest can be verified by

N
 twice, and if the resulting value is the same as the public key,
...
, as in Figure 2, an attacker would be able to generate a signature for a message

th digit of 2 by applying
 to
th digit of 0 as this would require finding some value
...
𝑠𝑠𝑘𝑘
 H
which would not be feasible as long as
k
...
𝐻𝐻(𝑦𝑦) = 𝑠𝑠𝑘𝑘
 ...
 twice. An attacker could not, however, generate a signature for a message

 once or to a message digest with a
...
checksum of the message digest and signs the checksum along with the digest. For an
message digest, the checksum is computed as
. The checksum is designed so
...
Signature  H6(x0)  H3(x1)  H15(x2)  H(x3)  H14(x4)  H9(x5)
...
H15(x0)  H15(x1)  H15(x2)  H15(x3)  H15(x4)  H15(x5)  H15(x6)  H15(x7)
...
ℎ2
 ...
 is the same as h0−7, then k2 may be used to verify the message signature.
′
= 𝐻𝐻(ℎ01||ℎ23
...
kb,8  kb,9  kb,10 kb,11

kb,12 kb,13  kb,14 kb,15
...
Leighton-Micali Signatures (LMS) Parameter Sets
...
eXtended Merkle Signature Scheme (XMSS) Parameter Sets
...
Random Number Generation for Keys and Signatures
...
Algorithm 10': XMSS'_keyGen
  ...
  Input: level L, tree t,
         public key of top-level tree PK_MT (if L
...
XMSS^MT external device keygen
  ...
  for each bottom-level key pair to be created {
    ...
    // key t of the top-level tree. If it was not, then try again.
 t ) {
    while ( getIdx(SigPK[t])
      t = getIdx(SigPK[t]) + 1;
      PK[t] = XMSS'_keygen(0, t, PK_MT);
      SigPK[t] = XMSS'_sign(getRoot(PK[t]));
    }
...
Huelsing A, Butin D, Gazdag S, Rijneveld J, Mohaisen A (2018) XMSS:
...
McGrew D, Curcio M, Fluhrer S (2019) Leighton-Micali Hash-Based
...
National Institute of Standards and Technology (2019) Post-Quantum
...
McGrew D, Kampanakis P, Fluhrer S, Gazdag S, Butin D, Buchmann J
...
Genêt A, Kannwischer MJ, Pelletier H, McLauchlan A (2018) Practical Fault
Injection Attacks on SPHINCS. Cryptology ePrint Archive, Report
2018/674. https://eprint.iacr.org/2018/674

Castelnovi L, Martinelli A, Prest T (2018) Grafting trees: A fault attack
...
Fluhrer S (2017) Further Analysis of a Proposed Hash-Based Signature
Standard. Cryptology ePrint Archive, Report 2017/553.
https://eprint.iacr.org/2017/553.pdf

Buchmann J, Dahmen E, Hulsing A (2011) XMSS – A Practical Forward
...
Merkle RC (1979) Security, Authentication, and Public Key Systems. PhD
...
National Institute of Standards and Technology (2001) Security
...
Chen L, Jordan S, Liu Y-K, Moody D, Peralta R, Perlner RA, Smith-Tone D
...
Sotirov A, Stevens M, Appelbaum J, Lenstra A, Molnar D, Osvik DA, de
...
Housley R (2019) Hash Of Root Key Certificate Extension. (Internet
